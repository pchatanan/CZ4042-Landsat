...
num_hidden_list = [5, 10, 15, 20, 25]
num_data = X.shape[0]

time_list = []
for i, num_hidden in enumerate(num_hidden_list, 1):
    # Create the model
    # input
    ...
    with tf.Session() as sess:
        ...
        plt.savefig('partA_Qn3a_(hidden{0}).png'.format(num_hidden))

# Plot for Question 3b
fig = len(num_hidden_list) + 1
plt.figure(fig)
plt.plot(num_hidden_list, time_list, 'b')
plt.xlabel('Number of hidden neurons')
plt.ylabel('Time taken per epoch (sec)')
plt.savefig('partA_Qn3b.png')
